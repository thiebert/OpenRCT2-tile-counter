...
function CreateCountWindow() {
    var window = new Oui.Window("Count Window");
    window.setWidth(300);

    var raisedBox = new Oui.GroupBox("Raised Tiles");
    window.addChild(raisedBox);
    var raisedDisclaimer = new Oui.Widgets.Label("Total count of all tiles with the Surface object above 0");
    raisedBox.addChild(raisedDisclaimer);

    var raisedLabel = new Oui.Widgets.Label(GetRaisedTileCount().toString());
    raisedBox.addChild(raisedLabel);

    var blackedBox = new Oui.GroupBox("Non-Blacked Tiles");
    window.addChild(blackedBox);

    var blackedDisclaimer = new Oui.Widgets.Label("Total count of all tiles that are not blacktiled");
    blackedBox.addChild(blackedDisclaimer);

    var blackedLabel = new Oui.Widgets.Label(GetBlackedTileCount().toString());
    blackedBox.addChild(blackedLabel);

    var unbuiltBox = new Oui.GroupBox("Built Tiles");
    window.addChild(unbuiltBox);

    var unbuiltDisclaimer = new Oui.Widgets.Label("Total count of all tiles where Surface > 0 OR objects built above blacktiles");
    unbuiltBox.addChild(unbuiltDisclaimer);

    var unbuiltLabel = new Oui.Widgets.Label(GetUnbuiltTileCount().toString());
...
function GetBlackedTileCount() {
    ...
    for (var x = 0; x < map.size.x; x++) {
        for (var y = 0; y < map.size.y; y++) {
            ...
            if (getSurface(tile).baseHeight <= 2 && (tile.numElements == 1 || firstElement.baseHeight <= 2 && firstElement.type === "large_scenery")) {
                count++;
            }
        }
    }
...
function GetUnbuiltTileCount() {
    ...
    for (var x = 0; x < map.size.x; x++) {
        for (var y = 0; y < map.size.y; y++) {
            var tile = map.getTile(x, y);
            var firstElement = getFirstElement(tile);
            if (getSurface(tile).baseHeight <= 2 && (tile.numElements == 1 || firstElement.baseHeight <= 2 && firstElement.type === "large_scenery" && tile.numElements == 2)) {
                count++;
            }
        }
    }
...
function getSurface(tile) {
    for (var index = 0; index < tile.numElements; index++) {
        if (tile.elements[index].type === "surface") return tile.elements[index];
